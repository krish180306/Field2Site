...
**Q: Why use a Connection Pool instead of single connections?**
> Opening a new DB connection per request is expensive. The pool keeps 10 "warm" connections ready, drastically reducing latency under concurrent traffic.

**Q: How are file uploads handled?**
> Multer streams image chunks to `server/uploads/` on disk and stores only the relative URL path in the database. This keeps the DB lightweight while serving images as static files.

**Q: How does the booking transaction work?**
> Creating a booking uses `pool.getConnection()` + `beginTransaction()`. Both the `BOOKING` insert and `FARM/CONSTRUCTION_EQUIPMENT` status update run atomically — if either fails, the whole operation rolls back.

**Q: What happens when a booking expires?**
> A `setInterval` background task runs every minute. It queries for bookings whose `End_Date <= NOW()` and status is still `Pending` or `Confirmed`, marks them `Completed`, and restores the equipment to `Available`.

**Q: What happens if a Host is deleted?**
> `ON DELETE CASCADE` propagates the deletion through all child records: equipment → bookings → payments → reviews.

**Q: Why separate FARM and CONSTRUCTION equipment tables?**
> They share common columns today but have different domain attributes that may diverge (e.g., soil compatibility for farm, lift height for construction). Separate tables keep queries targeted and schema clean.

---

## 🚀 Future Roadmap
- **Real Payment Gateway**: Integrate Stripe or Razorpay instead of mode selection.
- **OTP Storage**: Move from in-memory `Map` to Redis for multi-instance resilience.
- **Image Storage**: Migrate from local disk to AWS S3 / Cloudinary.
- **JWT Authentication**: Add signed tokens for stateless, secure session management.
- **Live Tracking**: Integrate GPS APIs for real-time equipment delivery tracking.
- **AI Recommendation**: Suggest equipment based on the Buyer's booking history and season.
